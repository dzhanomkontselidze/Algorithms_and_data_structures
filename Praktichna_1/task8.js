...
// завдання 7
function task7_RemoveMaxMinDuplicates(arr) {
    ...
    return arr.filter(item => {
        if (item === max) {
            if (!maxFound) { maxFound = true; return true; }
            return false;
        }
        if (item === min) {
            if (!minFound) { minFound = true; return true; }
            return false;
        }
        return true;
    });
...
// завдання 8
function task8_ElementsBetweenAverages(arr1, arr2) {
...
console.log("- Додатні та від'ємні:",task6_SplitPosNeg(array1));
console.log("- Без дублікатів max/min:", task7_RemoveMaxMinDuplicates(array1));
console.log("- Елементи між середніми арифметичними:", task8_ElementsBetweenAverages(array1, array2));
